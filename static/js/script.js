...
if(mainNav){
  if(document.body.classList.contains('product-body')||document.querySelector('.shop-main')){
    /* Shop & product pages: solid nav at all times. */
    mainNav.classList.add("nav-scrolled");
  }else{
    /* All other pages (home, orders, about, etc.) use dark, readable nav text on a
       see-through bar that turns solid once you scroll past 60px. */
    mainNav.classList.add("nav-light");
    window.addEventListener("scroll",()=>{mainNav.classList.toggle("nav-scrolled",window.scrollY>60);},{passive:true});
  }
}
...
function refreshCartPrices(){
  ...
  fetch('/api/products/').then(r=>r.json()).then(d=>{
    const map={};(d.products||[]).forEach(p=>{map[p.id]={price:p.price,mrp:p.mrp,stock:p.stock,track:p.track};});
    let priceChanged=false,stockAdjusted=false;
    cart.forEach(it=>{
      ...
      if(np!==it.price){it.price=np;priceChanged=true;}
      it.mrp=nm;
      /* keep the cart's stock knowledge fresh */
      it.track=!!map[it.slug].track;
      it.stock=it.track?map[it.slug].stock:null;
    });
    /* clamp each product's TOTAL across all its lines down to its stock
       (fixes carts that already exceed stock, e.g. quick-add + weight variant) */
    const bySlug={};
    cart.forEach(it=>{if(it.slug)(bySlug[it.slug]=bySlug[it.slug]||[]).push(it);});
    Object.values(bySlug).forEach(lines=>{
      const st=lines[0].stock;
      if(!lines[0].track||typeof st!=='number')return;
      let total=lines.reduce((s,i)=>s+i.qty,0);
      for(let k=lines.length-1;k>=0&&total>st;k--){
        const cut=Math.min(lines[k].qty,total-st);
        lines[k].qty-=cut;total-=cut;stockAdjusted=true;
      }
    });
    /* drop items that are now fully out of stock */
    for(let i=cart.length-1;i>=0;i--){if(cart[i].qty<=0){cart.splice(i,1);stockAdjusted=true;}}
    if(priceChanged||stockAdjusted)renderCart();
    if(stockAdjusted)showToast('<i class="fa-solid fa-layer-group"></i> Cart updated to match available stock.',3500,'warn');
...
function addToCart(name,price,image,slug,mrp,weight,ratio,stock,track){
  const w=weight||'';
  /* stock cap only applies to inventory-tracked products */
  const limited=!!track&&typeof stock==='number'&&!isNaN(stock);
  /* All weight variants of a product share ONE stock pool, and the same product
     can sit on several cart lines (quick-add vs a weight pick). Cap on the TOTAL. */
  const slugTotal=slug?cart.filter(i=>i.slug===slug).reduce((s,i)=>s+i.qty,0):0;
  if(limited&&slugTotal>=stock){
    showToast(`<i class="fa-solid fa-layer-group"></i> Only ${stock} of ${name} left in stock`,3000,'warn');
    return false;
  }
  const existing=cart.find(i=>(i.slug&&i.slug===slug&&(i.weight||'')===w)||(!slug&&i.name===name));
  if(existing){existing.qty+=1;}
  else{cart.push({name,price:parseFloat(price),image,slug:slug||'',mrp:mrp?parseFloat(mrp):null,weight:w,ratio:ratio||1,qty:1,stock:limited?stock:null,track:!!track});}
  /* keep stock/track fresh on every line of this product so the cap stays consistent */
  if(limited&&slug){cart.forEach(i=>{if(i.slug===slug){i.stock=stock;i.track=true;}});}
  renderCart();
  /* No toast here — each caller shows exactly one success toast (avoids duplicates). */
  return true;
}
...
function renderCart(){
  ...
  cartItemsEl.querySelectorAll(".qty-btn").forEach(btn=>{btn.addEventListener("click",()=>{
    const idx=+btn.dataset.idx;const it=cart[idx];
    if(btn.dataset.action==="inc"){
      const slugTotal=it.slug?cart.filter(x=>x.slug===it.slug).reduce((s,x)=>s+x.qty,0):it.qty;
      if(it.track&&typeof it.stock==='number'&&slugTotal>=it.stock){
        showToast(`<i class="fa-solid fa-layer-group"></i> Only ${it.stock} left in stock`,2500,'warn');
        return;
      }
      it.qty+=1;
    }else{it.qty-=1;if(it.qty<=0)cart.splice(idx,1);}
    renderCart();
  });});
...
document.addEventListener("click",e=>{
  ...
  if(btn){
    ...
    // Quick-add uses the product's default size so it matches the product page line
    const w=btn.dataset.weight||'';
    const nm=btn.dataset.name+(w?` (${w})`:'');
    const ok=addToCart(nm,btn.dataset.price,btn.dataset.image,btn.dataset.slug||'',btn.dataset.mrp||null,w,1,btn.dataset.stock!==undefined&&btn.dataset.stock!==''?parseInt(btn.dataset.stock):null,btn.dataset.track==='1');
    openCart();
    if(ok)showToast(`<i class="fa-solid fa-circle-check"></i> ${nm} added to cart`);
...
if(heroAddCartBtn){
  heroAddCartBtn.addEventListener("click",()=>{const ok=addToCart(heroAddCartBtn.dataset.name,heroAddCartBtn.dataset.price,heroAddCartBtn.dataset.image,heroAddCartBtn.dataset.slug||'',heroAddCartBtn.dataset.mrp||null);openCart();if(ok)showToast(`<i class="fa-solid fa-circle-check"></i> ${heroAddCartBtn.dataset.name} added to cart`);});
}
...
/* ── Address book (accounts.Address) — pick a saved address at checkout ── */
function _addrToText(a){
  const parts=[a.line1];
  if(a.line2)parts.push(a.line2);
  parts.push(`${a.city} - ${a.pincode}, ${a.state}`);
  return parts.filter(Boolean).join('\n');
}
function _addrDisplay(a){
  return [a.line1,a.line2,`${a.city} - ${a.pincode}, ${a.state}`].filter(Boolean).join(', ');
}
async function _coLoadAddresses(){
  try{
    const d=await (await fetch('/api/addresses/')).json();
    const addrs=d.addresses||[];
    if(addrs.length){_coRenderAddrBook(addrs);return true;}
  }catch{}
  return false;
}
function _coRenderAddrBook(addrs){
  const cards=addrs.map(a=>`
    <div class="co-saved-card" style="position:relative">
      <button class="co-addr-del" data-id="${a.id}" title="Remove" style="position:absolute;top:10px;right:10px;background:none;border:none;color:#c62828;cursor:pointer;font-size:.92rem">&#128465;</button>
      <div style="font-weight:900;font-size:.92rem;color:#0d2b6b;margin-bottom:2px">${_esc(a.name)}${a.is_default?' <span style="font-size:.6rem;color:#15803d;background:#dcfce7;border-radius:6px;padding:2px 7px;vertical-align:middle">DEFAULT</span>':''}</div>
      <div style="font-size:.78rem;color:rgba(13,43,107,.45);font-weight:700;margin-bottom:8px">${_esc(a.phone)}</div>
      <div style="font-size:.82rem;color:rgba(13,43,107,.6);font-weight:600;line-height:1.55;margin-bottom:14px">${_esc(_addrDisplay(a))}</div>
      <button class="co-deliver-btn co-addr-deliver" data-id="${a.id}"><i class="fa-solid fa-check"></i> Deliver Here</button>
    </div>`).join('');
  document.getElementById('coAddrSection').innerHTML=`
    <div class="co-sec-head"><i class="fa-solid fa-location-dot"></i> Choose Delivery Address</div>
    <div style="display:flex;flex-direction:column;gap:12px">${cards}</div>
    <div style="text-align:center;margin-top:14px">
      <button id="coNewAddrBtn" class="co-saved-link">+ Add a new address</button>
    </div>`;
  const byId=id=>addrs.find(x=>String(x.id)===String(id));
  document.querySelectorAll('.co-addr-deliver').forEach(b=>b.addEventListener('click',()=>{
    const a=byId(b.dataset.id);
    if(a)_coSubmit(a.name,a.phone,_addrToText(a));
  }));
  document.querySelectorAll('.co-addr-del').forEach(b=>b.addEventListener('click',async()=>{
    b.disabled=true;
    try{await fetch(`/api/addresses/${b.dataset.id}/delete/`,{method:'POST',headers:{'X-CSRFToken':getCSRF()}});}catch{}
    if(!await _coLoadAddresses())_coRenderForm(false);   // none left → show the form
  }));
  document.getElementById('coNewAddrBtn').addEventListener('click',()=>_coRenderForm(false,true));
}
...
function _coRenderForm(prefill,fromBook){
  ...
  const backHtml=fromBook
    ?`<button id="coBackSaved" class="co-saved-link" style="margin-bottom:12px"><i class="fa-solid fa-arrow-left"></i> Back to saved addresses</button>`
  ...
  if(backBtn)backBtn.addEventListener('click',fromBook?_coLoadAddresses:_coRenderSaved);
  ...
  document.getElementById('coForm').addEventListener('submit',async e=>{
    ...
    // Save to the address book for next time (best-effort — never blocks the order)
    try{
      await fetch('/api/addresses/add/',{method:'POST',headers:{'Content-Type':'application/json','X-CSRFToken':getCSRF()},
        body:JSON.stringify({name,phone,line1:addr1,line2:area,city,state,pincode})});
    }catch{}
...
async function buildCheckoutModal(){
  ...
  if(!await _coLoadAddresses()){
    try{
      const data=await (await fetch('/api/saved-address/')).json();
  ...
  function doSearch(q) {
    ...
    if (matches.length === 0) {
      ...
      smResults.innerHTML = `<div class="sm-res-label">${matches.length} result${matches.length>1?'s':''} found</div>` +
        matches.map(p => {
          const soldOut = p.track && (p.stock||0) <= 0;
          return `
          <div class="sm-res-item" onclick="window.location.href='/product/?id=${p.id}'">
            <img src="${p.img}" alt="${p.name}" class="sm-res-img">
            ${soldOut?'<span class="sold-out-badge" style="left:10px;right:auto;top:50%;transform:translateY(-50%)">Sold Out</span>':''}
            ...
            ${soldOut
              ? `<button class="sm-res-add" disabled style="opacity:.5;cursor:not-allowed" onclick="event.stopPropagation()">Sold Out</button>`
              : `<button class="sm-res-add add-to-cart-btn" data-name="${p.name}" data-price="${p.price}" data-image="${p.img}" data-slug="${p.id}" data-mrp="${p.mrp||''}" data-stock="${p.stock!=null?p.stock:''}" data-track="${p.track?1:0}" data-weight="${p.weight||''}" onclick="event.stopPropagation()">ADD</button>`}
          </div>`;
        }).join('');
    }
  }
...
/* ── OWL CAROUSEL ── (only if jQuery + the Owl plugin are present on this page) */
...
/* ════════════════════════════════════════════════════════════
   PRODUCT-PAGE CODE (merged from the former product.js).
   The PDP render is a no-op on pages without .pdp-main.
   ════════════════════════════════════════════════════════════ */
...
(function(){
  ...
  /* Only show the badge pill when there's actually badge text (no empty coloured pill) */
  if(product.badge){badge.textContent=product.badge;badge.style.background=product.badgeColor||'';badge.style.display='';}
  ...
  /* ── Sold-out state (inventory-tracked DB products only) ── */
  const soldOut=product.trackInventory&&(product.stockQuantity||0)<=0;
  if(soldOut){
    addBtn.disabled=true;
    addBtn.classList.add('pdp-add-cart--soldout');
    addBtn.innerHTML='<i class="fa-solid fa-ban"></i>&nbsp; Sold Out';
    const imgBadge=document.getElementById('pdpImgBadge');
    if(imgBadge){imgBadge.textContent='Sold Out';imgBadge.style.background='#9aa0a6';imgBadge.style.display='';}
  }
  ...
  document.getElementById('pdpPlus').addEventListener('click',()=>{
    /* Don't let the customer pick more than we have in stock — and tell them why */
    if(product.trackInventory&&qty>=(product.stockQuantity||0)){
      showToast(`<i class="fa-solid fa-layer-group"></i> Only ${product.stockQuantity} left in stock`,2500,'warn');
      return;
    }
  ...
  addBtn.addEventListener('click',()=>{
    ...
    let added=0;
    for(let i=0;i<qty;i++){
      // stop at the first blocked add — addToCart already shows ONE "left in stock" warning
      if(!addToCart(product.name+' ('+selectedWeight+')',currentPrice,product.image,product.id,currentMrp,selectedWeight,ratio,product.stockQuantity,product.trackInventory))break;
      added++;
    }
    if(added>0){openCart();showToast('<i class="fa-solid fa-basket-shopping"></i> '+product.name+' added to cart!');}
  });
